package com.zax.aspen.storage.api.architecture
...
/**
 * 强制 storage-api 的目录按《技术架构》7.3 组织: 契约类型目录在前, 业务组目录在后
 *
 * Storage 是单业务边界服务, 契约类型目录直接承载契约文件, 不设业务组目录;
...
class ApiPackageStructureTest {
    ...
    fun `test sources live only in sanctioned test directories`() {
        val testRoot = File("src/test/kotlin").walkTopDown()
            .filter { it.isDirectory && it.path.endsWith("com/zax/aspen/storage/api") }
            .first()
