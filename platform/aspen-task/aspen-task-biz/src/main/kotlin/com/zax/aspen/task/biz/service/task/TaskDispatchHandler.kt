...
class TaskDispatchHandler(
    ...
    private fun dispatchRetry(execution: TaskExecutionEntity) {
        ...
        val brief = resolveRetryTenantBrief(marked)
        dispatchExecutor.submit { runAttemptSafely(definition, marked, brief) }
    }

    /**
     * 重派轮回查 Admin 取真实租户编码与名称
     *
     * 含 {tenantCode}/{tenantName} 占位符的 URL/头/体, 取值必须与首投路径同源:
     * 直接以 tenantId 兜底会让同一 executionId 的重试轮投到不同目标
     * (2026-09-13 审核修复); 回查失败或未命中时才降级 id 文本并告警
     *
     * @param execution 已置 RUNNING 的重派执行
     * @return 回查命中的租户简要信息, 失败时为 id 文本兜底
     */
    private fun resolveRetryTenantBrief(execution: TaskExecutionEntity): TenantBriefDto {
        val tenantId = execution.tenantId
        val brief = runCatching { tenantSourceClient.listEnabledTenants() }
            .getOrNull()
            ?.firstOrNull { it.tenantId == tenantId }
        if (brief == null) {
            log.warn("重试租户回查失败或未命中, 降级 id 兜底渲染: executionId={}, tenantId={}", execution.executionId, tenantId)
        }
        return brief ?: TenantBriefDto(tenantId = tenantId, tenantCode = tenantId.toString(), name = tenantId.toString())
    }
