 ...
 * 仅允许 http/https, 解析目标主机全部地址并拒绝环回、私有、链路本地、任意本地与
 * 0.0.0.0/8 保留段、组播、IPv6 本地唯一与 CGNAT 段, 内部目标必须经
 * aspen.task.http.allowed-internal-hosts 显式白名单放行; 校验通过返回原 URI 供客户端
 * 直用。DNS 解析与真实请求之间存在理论上的重绑定窗口, v1 接受该窗口并以
 * 「校验紧邻请求」缩小暴露面
 */
class TaskHttpGuard(
     ...
     * @return 环回、私有、链路本地、任意本地、0.0.0.0/8 其余保留段、组播、IPv6 本地唯一或 CGNAT 段时为 true
     */
    private fun isForbiddenAddress(address: InetAddress): Boolean {
        ...
        // 0.0.0.0/8 全段拒绝: isAnyLocalAddress 只覆盖精确 0.0.0.0, 该段其余地址
        // 在 Linux 上通常本地可达, 与文档「拒保留地址」声明对齐 (2026-09-13 审核修复)
