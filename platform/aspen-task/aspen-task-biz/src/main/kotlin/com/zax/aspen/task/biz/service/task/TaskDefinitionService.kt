...
class TaskDefinitionService(
    ...
    fun createTask(command: TaskSaveDTO): TaskVO {
        ...
        synchronizer.syncSchedule(saved)
    ...
    fun updateTask(definitionId: Long, command: TaskSaveDTO): TaskVO {
        ...
        // 重排走 rescheduleJob 原子替换: 失败抛出使 DB 回滚且旧 Trigger 原样保留,
        // 等待下次变更或对账修复 (先删后建失败会静默停摆, 已在 2026-09-13 审核修复中弃用)
        synchronizer.syncSchedule(updated)
    ...
    private fun validate(command: TaskSaveDTO) {
        ...
        if (command.tenantScope == TaskTenantScope.SELECTED_TENANTS) {
            ...
            require(command.tenantIds.size <= MAX_TENANT_SELECTION) {
                "指定租户圈定最多 ${MAX_TENANT_SELECTION} 个 (当前 ${command.tenantIds.size})"
            }
        }
        ...
        // 请求头键值长度兜底校验: Bean Validation 无法直接约束 Map 的键值内容,
        // 超长值直落 JDBC 报错会渲染 500 而非 4xx (2026-09-13 审核修复)
        command.headers.forEach { (name, value) ->
            require(name.length <= MAX_HEADER_NAME_LENGTH) { "请求头名称过长 (上限 $MAX_HEADER_NAME_LENGTH): $name" }
            require(value.length <= MAX_HEADER_VALUE_LENGTH) {
                "请求头值过长 (上限 $MAX_HEADER_VALUE_LENGTH): $name"
            }
        }
    }
    ...
    private companion object {
        /** 指定租户圈定的清单上限, 与 TaskSaveDTO.tenantIds 的 @Size 同源 */
        const val MAX_TENANT_SELECTION = 1000

        /** 请求头名称长度上限 */
        const val MAX_HEADER_NAME_LENGTH = 128

        /** 请求头值长度上限 */
        const val MAX_HEADER_VALUE_LENGTH = 2000
