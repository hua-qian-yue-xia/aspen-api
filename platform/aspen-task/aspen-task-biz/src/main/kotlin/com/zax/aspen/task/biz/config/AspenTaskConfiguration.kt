...
class AspenTaskConfiguration {
    ...
    fun taskDispatchExecutor(properties: AspenTaskProperties): ThreadPoolTaskExecutor =
        ThreadPoolTaskExecutor().apply {
            ...
            // 拒绝策略必须 CallerRuns: 队列打满后回退到调用线程 (Quartz 线程) 执行, 形成
            // 有界阻塞的自然背压; 若用默认 AbortPolicy, submit 抛异常会中途夭折 dispatchRound,
            // 已插入的 RUNNING 行无人投递, 最终被僵尸回收误标 interrupted (2026-09-13 审核修复)
