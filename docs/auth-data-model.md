...
> 文档状态：三层模型（端注册 / 端×登录方式 / 主体凭据）、4 表结构与 Principal SPI 契约已定稿；首批落地管理端密码登录全链闭环（图形/三方行为验证码闸门、刷新轮换、登出吊销、网关 JWT 收紧），第三方登录、设备与合作方端为预留扩展位  
> 文档基线：2026-09-13  
...
Aspen Auth 是平台级统一认证服务，负责登录协议编排、令牌签发与刷新、验证码闸门与登录审计；用户、凭证与权限的主体数据归各端用户域（管理端为 Admin UPM），Auth 经 Principal SPI 访问，自身不建立第二套用户数据。代码位于：
...
platform/aspen-auth/aspen-auth-biz/src/main/resources/db/migration/          # V001 auth_session + auth_login_log
aspen-common/aspen-common-security/                                          # 客户端配置快照 SDK 与业务进程最小信任链
```
...
| 层 | 表 | 回答的问题 | 所在库 |
| --- | --- | --- | --- |
| 端注册 | `sys_auth_client` | 谁在调用：端身份、机器密钥摘要、令牌 TTL | `aspen_admin`（sys 迁移 V006） |
| 端×登录方式 | `sys_auth_login_method` | 这个端允许怎么登录：认证方式、验证码闸门、密码策略 | `aspen_admin`（sys 迁移 V006） |
| 主体凭据 | `upm_user_credential` 等 | 主体是谁、凭据对不对 | 各用户域（Auth 不持有） |

外加 Auth 自有的会话与审计层（`aspen_auth` 独立 Schema，V001）：`auth_session`（跨端刷新会话权威）与 `auth_login_log`（登录审计）。端注册与登录方式是平台级配置，全体租户共用，不做租户隔离；租户身份来自主体（用户所属租户），不来自端。
...
| 领域 | 表 | 职责 |
| --- | --- | --- |
| 端 | `sys_auth_client` | 端注册：编码、类型、显示名、机器密钥摘要、访问/刷新令牌 TTL 覆盖、启停 |
| 端 | `sys_auth_login_method` | 端×登录方式策略：允许的认证方式、验证码闸门、首登强制改密、密码有效期、方式专属配置 |
...
### 3.3 `auth_session`（`aspen_auth` V001）
...
### 3.4 `auth_login_log`（`aspen_auth` V001）
...
| `AuthLoginResult` | 见 3.4 | 登录结果分类 |

## 5. 令牌与会话语义

- 访问令牌为 JWT，RS256 签名；私钥 PKCS#8 base64 只从环境变量注入（`ASPEN_AUTH_JWT_PRIVATE_KEY` + `ASPEN_AUTH_JWT_KEY_ID`），公钥经 Auth 的 `/internal/auth/jwks` 以 JWKS 分发，Gateway 用 `kid` 选钥，轮换期可同时发布多把公钥。
...
| 操作 | 请求 | 响应语义 |
| --- | --- | --- |
| `verifyPassword` | account + secret 明文 | OK（附主体）/ NOT_FOUND / BAD_CREDENTIALS / LOCKED；失败计数与锁定判定在用户域（`upm_user_credential.failed_attempt_count/locked_until`） |
...
`UserPrincipalDto` 最小集：`principalId`、`displayName`、`status`（ENABLED/DISABLED/LOCKED）、`tenantId`（可空）、`mustChangePassword`。SPI 是跨服务 HTTP 契约，破坏性变更升主版本；新增用户域（app 服务、设备注册表）通过新增实现 + Auth 路由项接入，不改引擎。

## 7. 配置分发

端注册与登录方式经「版本信封 + Pub/Sub」分发（同路由快照语义）：

- 权威源 = `aspen_admin` 的 sys 两表；Admin 在事务提交后与启动时全量构建快照发布，Auth 只读消费，Redis 可随时全量重建。
- Key 与频道常量 `AuthClientContract` 位于 `aspen-common-security`（auth 与 admin 的 biz 直接依赖该 SDK，无 api 模块引用需求，不拆纯契约模块）：`aspen:{env}:auth:clients`（信封）、`:version`（INCR 计数器）、`:refresh`（频道）。
- 发布使用 common-cache 的版本守卫原语（Lua CAS，仅当信封版本新于在途版本才落盘并广播），并发发布不再出现旧盖新；路由快照分发同步切换到该原语。

## 8. 安全边界

- `sys_auth_client.secret_hash`、`auth_session.refresh_token_hash` 只存不可逆摘要；明文密钥仅在创建/轮换时展示一次。
...
- `sys_auth_login_method` 行随实现落地：只种 v1 真实现的行（`aspen-admin-web` 端 + PASSWORD/SLIDER 方式），不预填没有代码支撑的方式。
- `auth_login_log` 只追加；会话数据不含凭据明文；跨端会话吊销是管理面能力（后续批次）。

## 9. Schema 管理

沿用纯 SQL 目录约定（无 Flyway）：`sys_auth_client`/`sys_auth_login_method` 落 Admin sys 迁移 V006（全局版本序列，当前最高 V005）；`auth_session`/`auth_login_log` 落 `aspen-auth-biz` 的 `db/migration/V001`。`aspen_auth` 独立建库，部署侧手工创建（同 `aspen_admin`/`aspen_task` 约定）。UPM 既有 `upm_user_session`/`upm_login_log` 降级废弃（保留表结构不动，不再写入，见《Admin UPM 数据模型》）。
