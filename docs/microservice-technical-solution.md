...
- 路由定义的权威源是 Admin 的 `sys_route`（全局引用数据）；Gateway 不访问业务数据库，路由经 Redis 分发，Redis 只是可随时全量重建的分发介质。
- Admin 在路由变更事务提交后与自身启动时，把全部启用路由构建为带单调递增版本号的 JSON 信封，一条 `SET` 原子替换 Redis 单 Key（无删除窗口），再经 Pub/Sub 频道携带版本号通知各 Gateway 实例。
- Gateway 通过只读 `RouteDefinitionRepository` 消费：启动与收到通知时读取快照，与本地版本比对防乱序；解析失败的单条路由跳过并告警，不阻塞整体刷新。
- Gateway 运行期不访问 Redis，已加载路由不受 Redis 故障影响；Redis 不可用时冷启动以空路由启动并报健康 DOWN，配置文件静态路由仍合并生效作为兜底。
- 禁止 Gateway actuator 直写路由，数据库是唯一写入通道；变更通知不持久，断线期间的发布靠下次变更或重启自愈，周期对账在统一任务服务建立后接入。
